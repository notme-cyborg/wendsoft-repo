...
<p>halaman ini akan will do a system check to make sure your configuration looks correct. We'll add your first user on the next page. </p>
  ...
  <tr {!! ($start_settings['php_version_min']) ? ' class="success"' : ' class="danger"' !!}>
    ...
    <tr {!! ($start_settings['url_valid']) ? ' class="success"' : ' class="danger"' !!}>
      ...
      <td>
        ...
        @else
          Uh oh! wendsoft thinks your URL is {{ $start_settings['url_config'] }}, but your real URL is {{ $start_settings['real_url'] }}
    ...
    <tr {!! ($start_settings['prod']) ? ' class="success"' : ' class="warning"' !!}>
      ...
      <td>
        ...
        @else
          Your app is set <code>{{ $start_settings['env'] }}</code> instead of <code>production</code> mode. If you're not planning on developing on wendsoft, please update your <code>APP_ENV</code> settings in your  <code>.env</code> file to <code>production</code>.
        @endif
      </td>
    </tr>
